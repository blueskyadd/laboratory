<template>
    <div class="management_overdue body_main">
        <header class="testMethods_index_header">
            <h3>已过计量期设备数</h3>
            <span class="goBack underline" @click="$router.back(-1)">返回</span>
            <Search @searchDetail='searchGauger_outmeteringList' class="Taskreview_header_Search" :placeholderTexe = 'placeholderTexe'/>
        </header>
...
export default {
    ...
    methods:{
        ...
        handleCurrentChange(pageNumber) {
            ...
            this.isLoading = true;
            !this.isSearch?this.getGauger_outmeteringList(pageNumber):this.searchGauger_outmeteringList(this.searchText,pageNumber);
        },
        searchGauger_outmeteringList(data,pageNumber){
            pageNumber = pageNumber ? pageNumber : 1;
            this.isLoading = true;
            this.searchText = data;
            this.isSearch = true;
            this.currentPage = 1;
            this.$http.get(pageNumber == 1 ? this.$conf.env.getGauger_outmeteringList + '?search=' + data   + '&page_size=' +this.page_size : this.$conf.env.getGauger_outmeteringList + '?search=' + data + '&p=' +pageNumber +'&page_size=' +this.page_size ).then( res =>{
        ...
        getGauger_outmeteringList(pageNumber){
            this.isSearch = false;
            this.$http.get(pageNumber == 1 ? this.$conf.env.getGauger_outmeteringList + '?page_size=' +this.page_size : this.$conf.env.getGauger_outmeteringList + '?p=' +pageNumber +'&page_size=' +this.page_size ).then( res =>{
                this.isLoading = false;
                this.totalSum = res.data.count;
                this.tableData = res.data.results;
            }).catch(err =>{
    ...
    mounted(){
        this.getGauger_outmeteringList(1)
    },
     watch:{
        //根据当前输入页数跳转
        CurrentChange(newData, oldData){
            if(newData){
                ...
                !this.isSearch?this.getGauger_outmeteringList(this.CurrentChange):this.searchGauger_outmeteringList(this.searchText,this.CurrentChange);
            }
        },
    }
}
