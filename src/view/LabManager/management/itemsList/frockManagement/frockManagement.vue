<template>
    <div class="management_frockManagement">
        <div class="Search">
            <ul>
                <li>
            ...
            <div class="addequipment">
                <el-button  type="primary" @click="editquipment('新增工装', true)">新增</el-button>
            </div>
        </div>
        <el-table :data="tableData" :cell-style="changecolor"   style="width: 100%"  :row-class-name="tabRowClassName">
            ...
            <el-table-column prop="address" fixed='right' label="操作" header-align='center' align='center'>
                 <template slot-scope="scoped"><span class="underline lookmanagement"  @click="allocation(scoped)">查看</span><span class="underline lookmanagement"  @click="editquipment('编辑工装', false,scoped)">编辑</span><span class="underline deletemanagement" >删除</span> </template>
            </el-table-column>
        </el-table>
        <popUp ref="popUp" setWidth='45%' :popUptitle='popUptitle' class="popUp">
            <template>
                <ul>
                    <li>
                        <span>工装名称：</span>
                        <input type="text"  placeholder="填写工装名称">
                    </li>
                    <li>
                        <span>工装编号：</span>
                        <input   type="text" placeholder="填写工装编号">
                    </li>
                    <li >
                        <span>工装负责人：</span>
                        <el-select v-model="value" placeholder="选择工装负责人">
                            <el-option
                            v-for="item in options"
                            :key="item.value"
                            :label="item.label"
