<template>
    <div class="Taskreview_measure">
        <el-table :data="tableData" :cell-style="changecolor"   style="width: 100%"  :row-class-name="tabRowClassName">
            ...
            <el-table-column prop="address" fixed='right' label="合同上传" header-align='center' align='center'>
                <template slot-scope="scoped"><span class="underline"  @click="lookDetail(scoped)">上传</span> </template>
            </el-table-column>
        </el-table>
    </div>
</template>
...
export default {
    ...
    methods:{
        ...
        /**@name 页面跳转 */
        lookDetail(data){
            this.$router.push({name: 'updataMeasure' });
        },
        allocation(data){
            this.$router.push({name: 'applicationMeasure' });
        }
    }
}
</script>
