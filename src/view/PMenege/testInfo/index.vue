<template>
    <div class="management_testInfo body_main">
...
export default {
    ...
    methods:{
        ...
        searchPm_project_exfailLis(data,pageNumber){
            ...
            this.isSearch = true;
            this.currentPage = 1;
            this.$http.get(pageNumber == 1 ? this.$conf.env.getPm_project_exfailList + '?search=' + data + '&page_size=' +this.page_size : this.$conf.env.getPm_project_exfailList + '?search=' + data + '&p=' +pageNumber +'&page_size=' +this.page_size ).then( res =>{
        ...
        goEquipmentDetail(data){
            this.$router.push({path:'/LaboratoryManager/testInfoDetail',query:{"equipmentID": data.row.id}})
        },
        /**@name 分页 */
        handleCurrentChange(pageNumber) {
            ...
            this.isLoading = true;
            !this.isSearch?this.getPm_project_exfailList(pageNumber):this.searchPm_project_exfailLis(this.searchText,pageNumber);
        },
        getPm_project_exfailList(pageNumber){
            this.isSearch = false;
            this.$http.get(pageNumber == 1 ? this.$conf.env.getPm_project_exfailList + '?page_size=' +this.page_size : this.$conf.env.getPm_project_exfailList + '?p=' +pageNumber +'&page_size=' +this.page_size ).then( res =>{
                this.isLoading = false;
                this.totalSum = res.data.count;
                this.tableData = res.data.results;
            }).catch(err =>{
    ...
    mounted(){
        this.getPm_project_exfailList(1);
    },
    watch:{
        //根据当前输入页数跳转
        CurrentChange(newData, oldData){
            if(newData){
                ...
                !this.isSearch?this.getPm_project_exfailList(this.CurrentChange):this.searchPm_project_exfailLis(this.searchText,this.CurrentChange);
            }
        },
    }
}
